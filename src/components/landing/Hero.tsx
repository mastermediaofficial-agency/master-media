...
const DataStream = ({ index }: { index: number }) => {
  ...
  return (
    <motion.div
      className="absolute w-[1px] h-24 bg-linear-to-b from-transparent via-blue-500/40 to-transparent"
...
export default function Hero() {
  ...
  return (
    <section className="relative overflow-hidden flex items-center justify-center bg-black px-6 pb-15">
      {/* BACKGROUND LAYER */}
      ...
      <div className="relative z-10 max-w-5xl w-full text-center pt-24 md:pt-32">
        ...
        <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter leading-[1.1] mb-4">
          ...
          <motion.span
            ...
            className="ml-5 relative inline-block text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-white to-blue-400 cursor-default"
        ...
        >
          {[
            { label: "Total Reach", val: "10M+", icon: "📈" },
            { label: "Campaigns", val: "500+", icon: "🎯" },
            { label: "ROI Growth", val: "300%", icon: "🚀" },
          ].map((stat, i) => (
            <div
              key={i}
              className="p-6 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-md group hover:bg-white/[0.07] transition-all"
            >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-8">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="px-10 py-2.5 bg-blue-600 text-white rounded-full font-black text-lg relative overflow-hidden group shadow-xl"
          >
          ...
          <div className="flex items-center gap-3">
            <div className="flex -space-x-3">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-10 h-10 rounded-full border-2 border-black bg-slate-800 flex items-center justify-center text-[10px] font-bold text-white shadow-lg overflow-hidden"
                >
                  <img
